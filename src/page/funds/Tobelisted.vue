<template>
  <div class="wrapper">
    <div v-if="list.length <= 0 && !getStatus" style="margin-top:30vh" class="empty text-center">
      暂无订单信息!
    </div>
    <div v-if="list.length <= 0 && getStatus" class="empty text-center" style="margin-top:30vh">
        ...
        <li v-for="(item) in list" :key="item.key">
          <div class="order-info-box">
            <div class="order-title">
              <span @click="toDetail(item.newlistCode)" class="main">{{ item.stockName }}</span>
              <span class="secondary">({{ item.newlistCode }})</span>
              <span class="direction pull-right big-font">
                最新:
                <b v-if="item.buyOrderPrice == 0">-</b>
                <b v-else
                  :class="item.now_price - item.buyOrderPrice < 0 ? 'space green' : item.now_price - item.buyOrderPrice == 0 ? 'space' : 'space red'">{{ item.buyOrderPrice }}</b>
              </span>
            </div>
            <div class="order-info">
              ...
              <p class="clearfix" style="white-space:nowrap">
                ...
                <span class="col-xs-4">
                    数量:<b class="space">{{ item.orderNum }}</b>
                    <!-- 浮动盈亏:
